...
- H4 local 来源内容可改——写回 inbox/ 后重新 acquire,不改 raw/【用户】
- H5 Jira/Confluence 内容只读,编辑引导去源系统【用户】
...
- J8 **空态引导**:KB 为空或首次使用时引导下一步【用户】
- J9 **搜索结果反馈 → 黄金集闭环**:结果卡片 👍/👎,👎 查询沉淀为评测黄金集候选,
  与块 K 和现有 tests/eval 回归门形成活循环【用户】
...
- C5 **批量评审**:需要,非强需求;具体怎么批量需详细讨论【用户】
- B5 **查询历史/保存的查询**:需要,非强需求【用户】
