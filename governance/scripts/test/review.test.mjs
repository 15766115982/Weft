...
function makeKb() {
  ...
  const pageAbs = (slug) => path.join(kbRoot, 'wiki', 'topics', `${slug}.md`);
...
test('plan review_queue lists candidate pages only, with type/title/updated_at', () => {
  ...
  assert.equal(q[0].page, 'wiki/topics/cand-one.md');
  assert.equal(q[0].type, 'topic');
...
test('approve: candidate → approved + review log; wrong status/path fail loudly', () => {
  ...
  applyTopicPage(kbRoot, { slug: 'cand-one', title: 'Candidate One', sources: [RAW], candidate: true }, 'body');
  const out = approvePage(kbRoot, 'wiki/topics/cand-one.md');
  assert.equal(out.status, 'approved');
  assert.equal(readStatus(pageAbs('cand-one')), 'approved');
  assert.match(log(), /review \| approve \| wiki\/topics\/cand-one\.md \| via session/);
  assert.throws(() => approvePage(kbRoot, 'wiki/topics/cand-one.md'),
    /page status is "approved", expected "candidate"/);
  assert.throws(() => approvePage(kbRoot, 'wiki/index.md'), /page path must be wiki\/sources\|topics/);
  assert.throws(() => approvePage(kbRoot, 'wiki/archive/old.md'), /page path must be wiki\/sources\|topics/);
  assert.throws(() => approvePage(kbRoot, 'wiki/topics/ghost.md'), /page does not exist/);
  cleanup();
});
...
test('reject → sweep: page archived as status archived; second sweep is a no-op', () => {
  const { kbRoot, RAW, pageAbs, log, cleanup } = makeKb();
  applyTopicPage(kbRoot, { slug: 'cand-one', title: 'Candidate One', sources: [RAW], candidate: true }, 'body');
  rejectPage(kbRoot, 'wiki/topics/cand-one.md');
  assert.equal(readStatus(pageAbs('cand-one')), 'rejected');
  assert.match(log(), /review \| reject \| wiki\/topics\/cand-one\.md \| via session/);
  ...
  assert.match(log(), /govern \| auto:archive-rejected \| wiki\/archive\/cand-one\.md \| from wiki\/topics\/cand-one\.md/);
...
test('archive name collision: second archival of same basename lands at -2', () => {
  ...
  rejectPage(kbRoot, 'wiki/topics/dup.md');
  sweep(kbRoot);
...
test('backfill: viewer-style flip without log is recorded exactly once by sweep', () => {
  ...
  const r = sweep(kbRoot);
  assert.deepEqual(r.backfilled, [{ page: 'wiki/topics/cand-one.md', status: 'approved' }]);
  assert.match(log(), /review \| approve \| wiki\/topics\/cand-one\.md \| via viewer \(backfilled\)/);
  const r2 = sweep(kbRoot);
...
test('archivePage: approved page archived with note; candidate refused', () => {
  const { kbRoot, RAW, pageAbs, log, cleanup } = makeKb();
  applyTopicPage(kbRoot, { slug: 'ok-page', title: 'Approved Page', sources: [RAW] }, 'body');
  const out = archivePage(kbRoot, 'wiki/topics/ok-page.md', { note: 'superseded by payment-timeout' });
  assert.equal(out.page, 'wiki/archive/ok-page.md');
  assert.equal(out.from, 'wiki/topics/ok-page.md');
  ...
  assert.equal(archived.fields.status, 'archived');
  assert.match(log(), /govern \| archive \| wiki\/archive\/ok-page\.md \| from wiki\/topics\/ok-page\.md \| superseded by payment-timeout/);

  applyTopicPage(kbRoot, { slug: 'cand-one', title: 'Candidate One', sources: [RAW], candidate: true }, 'body');
  assert.throws(() => archivePage(kbRoot, 'wiki/topics/cand-one.md'),
...
test('merge: backlinks rewritten (forms + anchor + display preserved), sources unioned, loser archived', () => {
  ...
  writePage('wiki/topics/payment-timeout.md', { type: 'topic', status: 'approved', title: 'Payment Timeout' }, 'surviving body');
  writePage('wiki/sources/local-aaaa1111-pay.md', {
    type: 'source', status: 'approved', title: 'Pay', source_ref: RAW,
  }, 'See [[payment-retry]] and [[topics/payment-retry#budget|the budget section]].');
  ...
  assert.ok(linking.includes('[[topics/payment-timeout#budget|the budget section]]'), 'prefixed form + anchor + display preserved');
  const survivor = parseFrontmatter(fs.readFileSync(path.join(kbRoot, 'wiki', 'topics', 'payment-timeout.md'), 'utf8'));
  ...
  assert.match(log(), /govern \| merge \| wiki\/topics\/payment-timeout\.md \| from wiki\/topics\/payment-retry\.md \(archived, 1 backlink files\) \| dedup/);
...
test('merge validation: bad slug, same slug, missing page', () => {
  ...
  assert.throws(() => mergeTopics(kbRoot, 'payment-timeout', 'payment-timeout'), /merge requires two distinct topic slugs/);
  assert.throws(() => mergeTopics(kbRoot, 'ghost', 'payment-timeout'), /page does not exist: wiki\/topics\/ghost\.md/);
  cleanup();
});
...
test('M5 regression: plan reports topic provenance dangling after raw deletion', () => {
  ...
  assert.equal(orphans[0].page, 'wiki/topics/payment-timeout.md');
...
test('dangling_links: hand-made dead links reported, valid links not', () => {
  ...
  writePage('wiki/topics/broken.md', { type: 'topic', status: 'approved', title: 'Broken', sources: [RAW] },
    'Links to [[ghost-topic]] and [[topics/also-ghost#sec|display]].');
  const d = plan(kbRoot).dangling_links;
  assert.deepEqual(d, [
    { page: 'wiki/topics/broken.md', link: 'ghost-topic' },
    { page: 'wiki/topics/broken.md', link: 'topics/also-ghost' },
    { page: 'wiki/topics/payment-timeout.md', link: 'payment-retry' },
  ]);
  cleanup();
});
...
test('L4 regression: page with malformed status (no space) surfaces in errors, not silently dropped', () => {
  ...
  const abs = path.join(kbRoot, 'wiki', 'topics', 'ok-page.md');
  ...
  assert.ok(p.errors.some((e) => e.page === 'wiki/topics/ok-page.md' && /missing status field/.test(e.error)),
...
test("M1' regression: merge target with an unlogged viewer flip refuses until sweep", () => {
  ...
  flipStatus(path.join(kbRoot, 'wiki', 'topics', 'b-page.md'), 'candidate', 'approved'); // viewer: no log
  assert.throws(() => mergeTopics(kbRoot, 'a-page', 'b-page'),
    /unlogged review flip pending on this page; run sweep first: wiki\/topics\/b-page\.md/);
  sweep(kbRoot);
  const out = mergeTopics(kbRoot, 'a-page', 'b-page');
  assert.equal(out.action, 'merge');
  assert.match(log(), /review \| approve \| wiki\/topics\/b-page\.md \| via viewer \(backfilled\)/);
  cleanup();
});
...
test("M1' regression: merge source with an unlogged viewer flip refuses until sweep", () => {
  ...
  flipStatus(path.join(kbRoot, 'wiki', 'topics', 'c-page.md'), 'candidate', 'approved'); // viewer: no log
  assert.throws(() => mergeTopics(kbRoot, 'c-page', 'd-page'),
    /unlogged review flip pending on this page; run sweep first: wiki\/topics\/c-page\.md/);
  cleanup();
});
...
test("M1' regression: archive with an unlogged viewer flip refuses until sweep", () => {
  ...
  flipStatus(path.join(kbRoot, 'wiki', 'topics', 'e-page.md'), 'candidate', 'approved'); // viewer: no log
  assert.throws(() => archivePage(kbRoot, 'wiki/topics/e-page.md'),
    /unlogged review flip pending on this page; run sweep first: wiki\/topics\/e-page\.md/);
  sweep(kbRoot);   // backfills the approve, then archiving is legitimate
  const out = archivePage(kbRoot, 'wiki/topics/e-page.md');
...
test("M2' regression: merge refuses candidate pages on either side", () => {
  ...
  assert.throws(() => mergeTopics(kbRoot, 'cand-page', 'ok-page'),
    /merge involves a non-approved page \(status: candidate\); review candidates first \(approve or reject\): wiki\/topics\/cand-page\.md/);
  ...
  const cand = parseFrontmatter(fs.readFileSync(path.join(kbRoot, 'wiki', 'topics', 'cand-page.md'), 'utf8'));
...
test("L4' regression: merge rewrites the [[from.md]] extension form too", () => {
  ...
  writePage('wiki/topics/payment-timeout.md', { type: 'topic', status: 'approved', title: 'T' }, 'survivor');
...
test("L2' regression: dangling scan ignores links inside code fences and inline code", () => {
  const { kbRoot, RAW, writePage, cleanup } = makeKb();
  writePage('wiki/topics/code-page.md', { type: 'topic', status: 'approved', title: 'Code', sources: [RAW] },
    [
  ...
  const d = plan(kbRoot).dangling_links;
  assert.deepEqual(d, [
    { page: 'wiki/topics/code-page.md', link: 'ghost-real' },
    ...
    { page: 'wiki/topics/code-page.md', link: 'ghost-inline-fence' },
  ]);
  cleanup();
});
...
test("L3' regression: typo'd status surfaces in errors", () => {
  const { kbRoot, RAW, writePage, cleanup } = makeKb();
  writePage('wiki/topics/typo-page.md', { type: 'topic', status: 'apprved', title: 'Typo', sources: [RAW] }, 'body');
  const p = plan(kbRoot);
  assert.equal(p.review_queue.length, 0);
  assert.ok(p.errors.some((e) => e.page === 'wiki/topics/typo-page.md' && /illegal status "apprved"/.test(e.error)));
  cleanup();
});
